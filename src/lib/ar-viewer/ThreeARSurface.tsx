...
export const ThreeARSurface = ({
  ...
  useEffect(() => {
    ...
    return () => {
      if (label.parentElement === host) host.removeChild(label);
    };
  ...
  useEffect(() => {
    ...
    scene.add(
      Object.assign(new THREE.HemisphereLight(0xffffff, 0x444444, 1.2), {
        position: new THREE.Vector3(0, 1, 0),
      })
    );
    scene.add(
      Object.assign(new THREE.DirectionalLight(0xffffff, 0.7), {
    ...
    loader.load(
      ...
      (gltf: GLTF) => {
        ...
        if (prepareModelForRender(modelTemplate) === 0)
    ...
    controller.addEventListener('select', () => {
      if (!modelTemplate) {
        emitStatus('esperando-carga-modelo');
        return;
      }
      if (!reticle?.visible) {
        emitStatus('sin-superficie-detectada');
        return;
      }
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (e.touches.length === 1) {
        ...
        placedModel.rotation.y =
          g.startRotY + (touchAngle(t1, t2) - g.startAngle);
      }
    };
        ...
        if (!hitTestSourceRequested) {
          ...
          s.requestReferenceSpace('viewer')
            .then((rs: XRReferenceSpace) => {
              void s
                .requestHitTestSource?.({ space: rs })
                ?.then((src: XRHitTestSource) => {
                  hitTestSource = src;
                })
                ?.catch((err: unknown) => {
                  console.warn(
                    '[ThreeARSurface] requestHitTestSource failed:',
                    err
                  );
            ...
            .catch((err: unknown) => {
              console.warn(
                '[ThreeARSurface] requestReferenceSpace failed:',
                err
              );
        ...
        if (hitTestSource) {
          ...
          if (hits.length > 0) {
            ...
            if (pose) {
              ...
              emitStatus(
                g.isDragging && g.hasMoved
                  ? `modelo-colocado: ${modelLabel}`
  ...
  const statusLabel = (() => {
    ...
    const labels: Record<string, string> = {
      ...
      'modelo-cargado-listo-para-ar':
        'Tocá la superficie para colocar el modelo',
