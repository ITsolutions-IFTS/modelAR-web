...
export const ARPage = () => {
  ...
  const [trackingStatus, setTrackingStatus] = useState<ARTrackingStatus>('idle')

  useEffect(() => {
    if (!uid) { setPhase('error'); setErrorMsg('UID inválido'); return }
  ...
  if (phase === 'loading') {
    return (
      <div className="state-loading" style={{ height: 'calc(100dvh - var(--header-h))' }}>
        Cargando modelo...
      </div>
    )
  }
  ...
  if (phase === 'error' || !model || !downloadUrl) {
    return (
      <div className="state-error" style={{ height: 'calc(100dvh - var(--header-h))' }}>
        <p style={{ marginBottom: '1rem' }}>{errorMsg ?? 'No se pudo cargar el modelo'}</p>
  ...
  return (
    <div className="ar-layout">
      ...
      <aside className="ar-layout__panel">
        ...
        <h1 style={{ fontSize: '1rem', fontWeight: 700, letterSpacing: '-0.015em', marginBottom: '0.3rem', color: 'var(--text)' }}>
          {model.name}
        </h1>

        <p style={{ fontSize: '0.775rem', color: 'var(--text-3)', marginBottom: '0.75rem', letterSpacing: '0.01em' }}>
          @{model.user.username}
          {model.license && <span style={{ opacity: 0.6 }}> · {model.license.label}</span>}
        </p>

        {/* Estado AR */}
        <p style={{ fontSize: '0.775rem', color: 'var(--text-3)', marginBottom: '1rem' }}>
          Estado: <strong style={{ color: 'var(--accent)', fontWeight: 600 }}>{trackingStatus}</strong>
        </p>

        {/* Descripción */}
        {model.description && (
          <p style={{ fontSize: '0.825rem', color: 'var(--text-2)', lineHeight: 1.65, marginBottom: '1rem' }}>
        ...
        <div style={{ borderTop: '1px solid var(--line)', paddingTop: '1rem' }}>
          <p style={{ fontSize: '0.775rem', color: 'var(--text-3)', letterSpacing: '0.01em', marginBottom: '0.75rem' }}>
