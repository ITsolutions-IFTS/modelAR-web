...
export const SECTOR_META: Record<ITSector, SectorMeta> = {
  ecommerce: {
    label: 'Ecommerce',
    description: 'Tiendas que quieren que el comprador vea el producto en su espacio antes de comprar.',
    categories: [
  ...
  turismo: {
    label: 'Turismo',
    description: 'Municipios y museos que ponen QRs en monumentos y sitios históricos.',
    categories: [
  ...
  educacion: {
    label: 'Educación',
    description: 'Editoriales e instituciones que enriquecen materiales pedagógicos con modelos 3D.',
    categories: [
...
export function getBestThumbnail(model: SketchfabModel, targetWidth = 300): string {
  ...
  return (sorted.find((img) => img.width >= targetWidth) ?? sorted[sorted.length - 1]).url
}
